 ...
 *SH001            20241001   ONKARK           Validates/Approves CoLines and maintains COStop 
 *
 *
 */
...
/****************************************************************************************
 ...
 Date: 
 Description:
 * This script processes approval lines in M3. It retrieves workflow and order line data,
   validates input values, and inserts or updates records in the EXTOLN and OOLINE tables.
...
public class AddApprovalLine extends ExtendM3Transaction {
    ...
    /*
     * Updates LINE level CoStop value i.e OLSC in the order line table (OOLINE)
     */
    private void updateCoStopInOrderLine() {
        DBAction query = database.table("OOLINE").index("00").build();
        DBContainer container = query.getContainer();

        container.set("OBCONO", inCONO.toInteger()); // Company
        container.set("OBORNO", inORNO);             // Order number
        container.setInt("OBPONR", inPONR.toInteger()); // PO line number
        container.set("OBPOSX", inPOSX.toInteger());  // Line suffix

        query.readLock(container, updateCoStopCallBack);
    }

    /*
     * Callback function to update CoStop in OOLINE
     */
    Closure<?> updateCoStopCallBack = { LockedResult lockedResult ->
        lockedResult.set("OBOLSC", 1); // Set CoStop
    };
