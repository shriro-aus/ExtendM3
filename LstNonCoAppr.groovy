 ...
 * It processes records where the STOP status is '2'(cancel) and the HSTP value field equals '8'.
...
public class LstNonCoAppr extends ExtendM3Transaction {
    ...
    private void listOfrecords() {
        DBAction action = database.table('EXTOLN').index('00').selection('EXCONO', 'EXORNO', 'EXPONR', 'EXPOSX', 'EXSTOP', 'EXHSTP').build()
